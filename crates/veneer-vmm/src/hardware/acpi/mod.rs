...
fn current_oem_id() -> [u8; 6] {
    if let Some(p) = active::PROFILE.get() {
        let s = unsafe { core::ptr::addr_of!(p.hardware.firmware.spec.acpi_oem_id).read_unaligned() };
...
fn current_oem_table_id() -> [u8; 8] {
    if let Some(p) = active::PROFILE.get() {
        let s = unsafe { core::ptr::addr_of!(p.hardware.firmware.spec.acpi_oem_table_id).read_unaligned() };
        let bytes = s.as_bytes();
        if !bytes.is_empty() { return fit::<8>(bytes, b' '); }
        let m = unsafe { core::ptr::addr_of!(p.hardware.system.spec.product).read_unaligned() };
        let mb = m.as_bytes();
...
fn current_creator_id() -> u32 {
    if let Some(p) = active::PROFILE.get() {
        let s = unsafe { core::ptr::addr_of!(p.hardware.firmware.spec.acpi_creator_id).read_unaligned() };
        let bytes = s.as_bytes();
...
pub fn build(n_vcpus: usize) -> Result<Acpi, AcpiError> {
    ...
    // chain-load mode).
    let page = crate::platform::alloc_pages(ACPI_PAGES,
    )
...
pub unsafe fn build_at(host_dest: u64, guest_base: u64, n_vcpus: usize) -> Acpi {
    ...
    unsafe {
        ...
        (*hpet_ptr).base_address = GenericAddress {
            ...
            address: crate::hardware::devices::irq::hpet::HPET_BASE,
        };
    ...
    unsafe {
        ...
        *h.add(1) = 0; // Flags
        ...
        core::ptr::write_unaligned(h.add(0x18) as *mut u64, crate::hardware::devices::iommu::efr()); // EFR image (single source w/ MMIO reg)
