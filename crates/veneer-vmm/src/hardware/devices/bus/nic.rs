...
fn mac_bytes() -> [u8; 6] {
    ...
    if let Some(p) = active::PROFILE.get() {
        let mac_str = unsafe { core::ptr::addr_of!(p.hardware.network.instance.mac).read_unaligned() };
...
fn phy_id() -> u16 {
    active::PROFILE
        .get()
        .map(|p| unsafe { core::ptr::addr_of!(p.hardware.network.spec.phy_id).read_unaligned() })
        .filter(|&v| v != 0)
        .unwrap_or(0x67C9)
}
