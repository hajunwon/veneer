...
//!
//!     elapsed_pit = (rdtsc() - start_tsc) * PIT_FREQ_HZ / crate::infra::clock::tsc_freq::host_tsc_freq()
//!     counter_now = (initial - elapsed_pit) mod modulus
//!
//!   crate::infra::clock::tsc_freq::host_tsc_freq() is approximate (Ryzen 7000-series base ~4.7 GHz).
...
fn live_counter(idx: usize) -> u16 {
    ...
    // Multiply then divide is safe because PIT_FREQ_HZ < crate::infra::clock::tsc_freq::host_tsc_freq().
    let elapsed_pit = (elapsed_tsc / crate::infra::clock::tsc_freq::host_tsc_freq()) * PIT_FREQ_HZ
        + ((elapsed_tsc % crate::infra::clock::tsc_freq::host_tsc_freq()) * PIT_FREQ_HZ) / crate::infra::clock::tsc_freq::host_tsc_freq();
...
fn irq0_period_tsc() -> u64 {
    ...
    let modulus: u64 = if initial_raw == 0 { 0x10000 } else { initial_raw };
    modulus.saturating_mul(crate::infra::clock::tsc_freq::host_tsc_freq()) / PIT_FREQ_HZ
}
