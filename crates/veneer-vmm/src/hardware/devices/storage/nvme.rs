...
fn run_identify(prp1: u64, cdw10: u32, nsid: u32) {
    ...
    if t < NVME_TRACE_CAP {
        ...
        crate::sprintln!(
            ...
            cns, nsid, prp1, dst.is_null(), super::ns_present(nsid), nsze
        );
    }
    if dst.is_null() { return; }
...
fn nvme_vendor_id() -> u16 {
    active::PROFILE
        .get()
        .map(|p| unsafe { core::ptr::addr_of!(p.hardware.storage.spec.pci_id).read_unaligned() })
        .filter(|&v| v != 0)
...
fn build_identify_controller(buf: &mut [u8; 4096]) {
    ...
    let serial = active::PROFILE.get().map(|p| {
        let s = unsafe { core::ptr::addr_of!(p.hardware.storage.instance.serial).read_unaligned() };
        let b = s.as_bytes();
        let mut out = [b' '; 20];
        let n = b.len().min(20);
    ...
    let model = active::PROFILE.get().map(|p| {
        let m = unsafe { core::ptr::addr_of!(p.hardware.storage.spec.model).read_unaligned() };
        let b = m.as_bytes();
        let mut out = [b' '; 40];
        let n = b.len().min(40);
        out[..n].copy_from_slice(&b[..n]);
    ...
    let fw = active::PROFILE.get().map(|p| {
        let s = unsafe { core::ptr::addr_of!(p.hardware.storage.spec.firmware_rev).read_unaligned() };
        let b = s.as_bytes();
        let mut out = [b' '; 8];
        let n = b.len().min(8);
        out[..n].copy_from_slice(&b[..n]);
    ...
    let oui = active::PROFILE.get()
        .map(|p| unsafe { core::ptr::addr_of!(p.hardware.storage.spec.ieee_oui).read_unaligned() })
        .unwrap_or([0, 0, 0]);
    ...
    buf[516..520].copy_from_slice(&super::ns_count().to_le_bytes()); // NN = namespace count
    ...
    if let Some(p) = active::PROFILE.get() {
        let u = unsafe { core::ptr::addr_of!(p.hardware.system.instance.uuid).read_unaligned() };
        let ub = u.as_bytes();
        let n = ub.len().min(36);
...
fn build_identify_namespace(buf: &mut [u8; 4096], nsid: u32) {
    ...
    let (lba_count, lbads): (u64, u8) = if super::ns_present(nsid) {
        let bs = super::block_size(nsid);
        (super::block_count(nsid), bs.trailing_zeros() as u8)
    } else {
        (0, 9)
    };
...
fn do_rw(read: bool, slba: u64, nlb: u32, prp1: u64, prp2: u64, nsid: u32) -> u16 {
    ...
    let bs = super::block_size(nsid) as u64;
    if bs == 0 {
        return 0x01;
    }
    ...
    let ok = prp_for_each(prp1, prp2, total, |gpa, len| {
        ...
        while done < need {
            if pos == filled {
                ...
                if !super::read(nsid, lba, blocks, stage_ptr()) {
                    return false;
                }
                lba += blocks as u64;
                disk_left -= want;
    ...
    let ok = prp_for_each(prp1, prp2, total, |gpa, len| {
        ...
        while done < need {
            ...
            if filled == STAGE_BYTES {
                ...
                if !super::write(nsid, lba, blocks, stage_ptr() as *const u8) {
                    return false;
                }
                lba += blocks as u64;
                filled = 0;
