...
//! next_rip for CR intercepts; we step `crate::hypervisor::vmexit::lengths::CR_ACCESS`
//! bytes manually.
...
fn inject_gp(c: &mut crate::hypervisor::svm::vmcb::VmcbControl, error_code: u32) {
    ...
    match cr_index {
        ...
        3 => {
            ...
            if n < 16 {
                ...
                if new_val != 0x800000 {
                    let base = crate::hypervisor::vmexit::io::guest_ram_base();
                    if base != 0 {
                        unsafe { crate::guest_mem::dump_pagewalk(base, new_val, 0x3EF1102E); }
                        crate::guest_mem::dump_code_window(base, 0x3EF1102E, "cpudxe-fault");
                    }
                }
            }
            s.cr3 = new_val;
