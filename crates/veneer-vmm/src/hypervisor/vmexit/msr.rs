...
use crate::infra::arch::rdmsr as host_rdmsr;
...
pub unsafe fn handle_rdmsr(vmcb: *mut Vmcb, gprs: &mut GuestGprs) -> Action {
    ...
    if is_intel_only_msr(msr)
        && crate::hypervisor::vmexit::cpuid::profile_vendor() == crate::hypervisor::vmexit::cpuid::ProfileVendor::Amd
    {
    ...
    let (lo, hi) = match msr {
        ...
        idx::IA32_BIOS_SIGN_ID => split(
            active::PROFILE
                .get()
                .map(|p| unsafe {
                    core::ptr::addr_of!(p.hardware.cpu.spec.microcode_signature).read_unaligned()
                })
        ...
        idx::IA32_PLATFORM_ID => split(
            active::PROFILE
                .get()
                .map(|p| unsafe {
                    core::ptr::addr_of!(p.hardware.cpu.spec.platform_id).read_unaligned()
                })
                .unwrap_or(0),
        ),
        ...
        idx::AMD_TOP_MEM => split(crate::guest_mem::GUEST_RAM_BYTES),
        ...
        idx::IA32_TSC => split(crate::infra::clock::now()),
        ...
        idx::IA32_TSC_DEADLINE => split(crate::hardware::devices::irq::lapic::tsc_deadline_raw()),
        ...
        0x0000_0830 => {
            let lo = crate::hardware::devices::irq::lapic::read_register(0x300) as u64;
            let hi = crate::hardware::devices::irq::lapic::read_register(0x310) as u64;
            split((hi << 32) | lo)
        }
        0x0000_0800..=0x0000_083F => {
            let off = ((msr - 0x0000_0800) << 4) as u32;
            split(crate::hardware::devices::irq::lapic::read_register(off) as u64)
        }
...
pub unsafe fn handle_wrmsr(vmcb: *mut Vmcb, gprs: &mut GuestGprs) -> Action {
    ...
    match msr {
        idx::EFER => {
            ...
            s.efer = val | crate::infra::arch::EFER_SVME;
        }
        idx::KVM_SYSTEM_TIME_NEW => {
            crate::hardware::devices::kvmclock::handle_wrmsr_system_time(val);
        }
        idx::KVM_WALL_CLOCK_NEW => {
            crate::hardware::devices::kvmclock::handle_wrmsr_wall_clock(val);
        }
        idx::STAR           => s.star = val,
        ...
        idx::IA32_TSC_DEADLINE => {
            ...
            crate::hardware::devices::irq::lapic::set_tsc_deadline(val, crate::infra::clock::now());
        }
        // ───── x2APIC MSR window (0x800..0x83F) ───────────────────────
        0x0000_0830 => {
            // ICR: atomic 64-bit. Destination (high 32) then command (low
            // 32) — writing the low word is what triggers IPI delivery.
            crate::hardware::devices::irq::lapic::write_register_width(0x310, (val >> 32) & 0xFFFF_FFFF, 4);
            crate::hardware::devices::irq::lapic::write_register_width(0x300, val & 0xFFFF_FFFF, 4);
        }
        0x0000_083F => {
            // SELF IPI (x2APIC-only): vector in bits 7:0.
            crate::hardware::devices::irq::lapic::self_ipi((val & 0xFF) as u8);
        }
        0x0000_0800..=0x0000_083F => {
            let off = ((msr - 0x0000_0800) << 4) as u32;
            crate::hardware::devices::irq::lapic::write_register_width(off, val & 0xFFFF_FFFF, 4);
        }
        _ => {
