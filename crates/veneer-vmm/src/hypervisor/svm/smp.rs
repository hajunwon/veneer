...
use crate::hypervisor::vmexit::{self, Action};
use crate::hardware::identity::profile;
use crate::hypervisor::svm::vmcb::Vmcb;
use crate::hypervisor::svm::vmrun;
...
pub fn probe() -> Result<SmpInfo, SmpError> {
    let (total, enabled) = crate::platform::mp_processor_count().map_err(|_| SmpError::QueryFailed)?;
    let bsp = crate::platform::mp_who_am_i().map_err(|_| SmpError::QueryFailed)?;
    Ok(SmpInfo {
        total,
        enabled,
        bsp_index: bsp,
...
pub fn alloc_per_ap_hsave(n_aps: usize) -> Result<heapless_vec::Vec<u64, 32>, SmpError> {
    ...
    for _ in 0..n_aps {
        let p = crate::platform::alloc_pages(1,
        )
...
pub fn run_ap_probe() -> Result<ApReports, SmpError> {
    let (_total, enabled) = crate::platform::mp_processor_count().map_err(|_| SmpError::QueryFailed)?;
    if enabled <= 1 {
        return Ok(ApReports::empty());
    }
    let mut reports = ApReports::empty();
    let arg = &mut reports as *mut ApReports as *mut c_void;
    crate::platform::mp_startup_all_aps(ap_callback, arg, 5)
...
pub fn cleanup_ap_svm() -> Result<[u64; MAX_AP_SLOTS], SmpError> {
    let (total, enabled) = crate::platform::mp_processor_count().map_err(|_| SmpError::QueryFailed)?;
    crate::sprintln!("[svm-cleanup]   processors: total={} enabled={}", total, enabled);
    let mut efers = [0u64; MAX_AP_SLOTS];
    if enabled <= 1 {
        return Ok(efers);
    }
    let arg = efers.as_mut_ptr() as *mut c_void;
    crate::sprintln!("[svm-cleanup]   startup_all_aps (blocking, 5s timeout)");
    crate::platform::mp_startup_all_aps(ap_svm_cleanup_callback, arg, 5)
        .map_err(|_| SmpError::QueryFailed)?;
    crate::sprintln!("[svm-cleanup]   startup_all_aps returned");
    Ok(efers)
}
...
pub fn run_ap_vmrun(setup: &mut ApVmRunSetup) -> Result<(), SmpError> {
    ...
    crate::platform::mp_startup_all_aps(ap_vmrun_callback, arg, 10)
        .map_err(|_| SmpError::QueryFailed)?;
    Ok(())
}
