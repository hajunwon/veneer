...
pub fn report_profile(p: &profile::Profile) {
    ...
    sprintln!("[prof]   cpu     : brand=\"{}\" vendor=\"{}\" hide_bit={} hide_leaf={}",
        cpu.spec.brand.as_str(), cpu.spec.vendor.as_str(),
        cpu.spec.hide_hypervisor_bit, cpu.spec.hide_hypervisor_leaf);
    sprintln!("[prof]   smbios  : manufacturer=\"{}\" product=\"{}\"",
        smb.spec.manufacturer.as_str(), smb.spec.product.as_str());
    sprintln!("[prof]   smbios  : serial=\"{}\" uuid=\"{}\"",
        smb.instance.serial.as_str(), smb.instance.uuid.as_str());
    sprintln!("[prof]   network : mac=\"{}\"", net.instance.mac.as_str());
    sprintln!("[prof]   disk    : slot=\"{}\" model=\"{}\" serial=\"{}\"",
        disk.spec.slot.as_str(), disk.spec.model.as_str(), disk.instance.serial.as_str());
}
