...
pub fn run(path_s: &str) -> Result<()> {
    ...
    let smb = unsafe { core::ptr::addr_of!(p.hardware.system).read_unaligned() };
    let net = unsafe { core::ptr::addr_of!(p.hardware.network).read_unaligned() };
    let disk = unsafe { core::ptr::addr_of!(p.hardware.storage).read_unaligned() };

    println!("==== profile: {} ====", name.as_str());
    println!("  cpu.brand        : {}", cpu.spec.brand.as_str());
    println!("  cpu.vendor       : {}", cpu.spec.vendor.as_str());
    println!("  cpu.hide_hyper_b : {}", cpu.spec.hide_hypervisor_bit);
    println!("  cpu.hide_hyper_l : {}", cpu.spec.hide_hypervisor_leaf);
    println!("  smbios.product   : {} {}", smb.spec.manufacturer.as_str(), smb.spec.product.as_str());
    println!("  smbios.serial    : {}", smb.instance.serial.as_str());
    println!("  network.mac      : {}", net.instance.mac.as_str());
    println!("  disk.model       : {}", disk.spec.model.as_str());
    println!();
    ...
    let padded = cpu.spec.brand.to_padded();
    let rendered = std::str::from_utf8(&padded).unwrap_or("").trim_end_matches('\0');
    if rendered == cpu.spec.brand.as_str() {
        println!("  [+] CPUID brand string packs into the 48-byte leaf block");
    } else {
        println!("  [!] brand pack mismatch: {:?} vs {:?}", cpu.spec.brand.as_str(), rendered);
    }
