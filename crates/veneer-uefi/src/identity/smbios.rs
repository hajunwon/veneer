...
pub fn build() -> Result<Smbios, SmbiosError> {
    let page = boot::allocate_pages(AllocateType::AnyPages, MemoryType::RUNTIME_SERVICES_DATA, 1)
    ...
    let is_intel = crate::vmexit::cpuid::profile_is_intel();
    ...
    let (dimm_size_mb, dimm_speed_mts, dimm_count): (u16, u16, u8) = match profile {
        Some(p) => {
            ...
            let sz = unsafe { core::ptr::addr_of!((*m).size_mb_per_dimm).read_unaligned() };
            let sp = unsafe { core::ptr::addr_of!((*m).speed_mts).read_unaligned() };
            let dc = unsafe { core::ptr::addr_of!((*m).dimm_count).read_unaligned() };
            (
    ...
    unsafe {
        ...
        let vendor = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.bios_vendor), b"American Megatrends Inc."),
        ...
        let version = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.bios_version), b"1820"),
        ...
        let date = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.bios_date), b"09/05/2024"),
    ...
    unsafe {
        let uuid_bytes = match profile {
            Some(p) => parse_uuid_text(
                core::ptr::addr_of!(p.hardware.smbios.uuid).read_unaligned().as_bytes()),
            None => [0u8; 16],
        };
        b.write_byte(1);
        ...
        let manu = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.manufacturer), FB_MANUFACTURER),
        ...
        let prod = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.product), FB_PRODUCT),
        ...
        let ver = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.version), FB_VERSION),
        ...
        let serial = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.serial), FB_SERIAL),
    ...
    unsafe {
        ...
        let mut sc = 0u8;
        let manu = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.manufacturer), b"ASUSTeK COMPUTER INC."),
            None    => { let mut x = [0u8; 128]; x[..FB_MANUFACTURER.len()].copy_from_slice(FB_MANUFACTURER); x }
        };
        b.add_string(&manu[..ps_len(&manu)], &mut sc);
        let prod = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.product), b"ROG STRIX X670E-E GAMING WIFI"),
            None    => { let mut x = [0u8; 128]; x[..FB_PRODUCT.len()].copy_from_slice(FB_PRODUCT); x }
        };
        b.add_string(&prod[..ps_len(&prod)], &mut sc);
        ...
        let serial = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.board.baseboard_serial), b"230000000000000"),
            None    => { let mut x = [0u8; 128]; x[..FB_SERIAL.len()].copy_from_slice(FB_SERIAL); x }
        };
        b.add_string(&serial[..ps_len(&serial)], &mut sc);
        b.end_struct(sc);
    ...
    unsafe {
        ...
        let serial = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.board.chassis_serial), b"Default string"),
            None    => { let mut x = [0u8; 128]; x[..FB_SERIAL.len()].copy_from_slice(FB_SERIAL); x }
        };
        b.add_string(&serial[..ps_len(&serial)], &mut sc);
        b.end_struct(sc);
    ...
    unsafe {
        ...
        let cores = crate::vmexit::cpuid::effective_core_count();
        let (l1_per_core, l2_per_core, l3_total) = match profile {
            Some(p) => {
                let c = core::ptr::addr_of!((*p).hardware.cpu);
                let l1 = core::ptr::addr_of!((*c).cache_l1_per_core_kib).read_unaligned();
                let l2 = core::ptr::addr_of!((*c).cache_l2_per_core_kib).read_unaligned();
                let l3 = core::ptr::addr_of!((*c).cache_l3_total_kib).read_unaligned();
                (
                    if l1 != 0 { l1 } else { 64 },
    ...
    unsafe {
        let cores = crate::vmexit::cpuid::effective_core_count().min(255) as u8;
        ...
        let cpuid1 = crate::arch::cpuid(1);
        let eax_v1 = match profile {
            Some(p) => {
                let v = core::ptr::addr_of!((*p).hardware.cpu.cpuid_eax_v1).read_unaligned();
        ...
        let (family, max_mhz, cur_mhz) = match profile {
            Some(p) => {
                let c = core::ptr::addr_of!((*p).hardware.cpu);
                let fam = core::ptr::addr_of!((*c).smbios_processor_family).read_unaligned();
                let mx = core::ptr::addr_of!((*c).cpu_max_speed_mhz).read_unaligned();
                let cu = core::ptr::addr_of!((*c).cpu_current_speed_mhz).read_unaligned();
                (
        ...
        let socket = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.cpu.smbios_socket), b"AM5"),
            None    => { let mut x = [0u8; 128]; let s = b"AM5"; x[..s.len()].copy_from_slice(s); x }
        };
        b.add_string(&socket[..ps_len(&socket)], &mut sc);
        let vendor = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.cpu.vendor), b"AuthenticAMD"),
        ...
        let brand = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.cpu.brand), b"AMD Processor"),
            None    => { let mut x = [0u8; 128]; let s = b"AMD Processor"; x[..s.len()].copy_from_slice(s); x }
        };
        b.add_string(&brand[..ps_len(&brand)], &mut sc);
        let serial = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.smbios.serial), b"Unknown"),
    ...
    unsafe {
        ...
        let mem_manuf = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.memory.manufacturer), b"Corsair"),
            None    => { let mut x = [0u8; 128]; let s = b"Corsair"; x[..s.len()].copy_from_slice(s); x }
        };
        let mem_part = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.memory.part_number), b"CMK32GX5M2B5200C40"),
            None    => { let mut x = [0u8; 128]; let s = b"CMK32GX5M2B5200C40"; x[..s.len()].copy_from_slice(s); x }
        };
        let mem_ser0 = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.memory.serial0), b"5400123456789AB"),
            None    => { let mut x = [0u8; 128]; let s = b"5400123456789AB"; x[..s.len()].copy_from_slice(s); x }
        };
        let mem_ser1 = match profile {
            Some(p) => ps_or(core::ptr::addr_of!(p.hardware.memory.serial1), b"5400CAFE0123456"),
