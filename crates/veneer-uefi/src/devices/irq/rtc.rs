...
/// Using `crate::clock::now()` rather than raw RDTSC keeps the RTC in the
...
fn read_indexed(idx: u8) -> u8 {
    ...
    match idx & 0x7F {
        ...
        0x5B => ((crate::boot::guest_mem::high_size() >> 16) & 0xFF) as u8,
        0x5C => ((crate::boot::guest_mem::high_size() >> 24) & 0xFF) as u8,
        0x5D => ((crate::boot::guest_mem::high_size() >> 32) & 0xFF) as u8,
        _    => 0,
    }
}
...
fn mem_ext_64k_units() -> u16 {
    ...
    let low_ram = crate::boot::linux_loader::GUEST_RAM_BYTES;
...
fn pie_period_cycles() -> u64 {
    ...
    crate::clock::tsc_freq::host_tsc_freq() / hz
}
...
fn pie_sync() -> u64 {
    ...
    if PIE_ARMED_RATE.load(Ordering::Relaxed) != rate || PIE_NEXT_AT.load(Ordering::Relaxed) == 0 {
        ...
        PIE_NEXT_AT.store(crate::clock::now().wrapping_add(period), Ordering::Relaxed);
    }
