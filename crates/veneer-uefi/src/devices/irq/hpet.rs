...
///
/// The elapsed-cycle source is `crate::clock::now()`, the same filtered,
...
fn main_counter() -> u64 {
    ...
    let host_freq = crate::clock::tsc_freq::host_tsc_freq();
...
pub fn write_register_width(offset: u32, val: u64, width: u8) {
    ...
    match aligned {
        ...
        0x0F0 => {
            ...
            HPET_BASE_TSC.store(crate::clock::now(), Ordering::Relaxed);
        }
