...
pub fn set_fb_base(gpa: u64) {
    ...
    if gpa < 0x8000_0000 || gpa + FB_SIZE > crate::acpi::PCIE_MCFG_BASE {
        crate::sprintln!("[vga ] framebuffer base 0x{:08X} outside MMIO window — not mapping", gpa);
        return;
    }
    let npt = crate::svm::npt::NptRoot { pml4_phys: 0, pdpt_phys: pdpt, coverage_bytes: 0 };
    match crate::svm::npt::map_range(&npt, gpa, buf, FB_SIZE) {
...
pub fn blit() {
    ...
    let min_interval = crate::clock::tsc_freq::host_tsc_freq() / 30;
