...
fn g2h(gpa: u64) -> *mut u8 {
    crate::boot::guest_mem::to_host_ptr(gpa)
}
...
fn port_read(po: u32) -> u32 {
    match po {
        ...
        0x24 => if backend::media_ready() { SIG_ATAPI } else { 0xFFFF_FFFF },
        0x28 => if backend::media_ready() { SSTS_PRESENT } else { 0 },
...
fn handle_scsi(cdb: &[u8; 16], ctba: u64, prdtl: u32) -> u32 {
    ...
    match cdb[0] {
        ...
        0x25 => {
            ...
            let last = backend::media_block_count().saturating_sub(1) as u32;
            let bs = backend::media_block_size();
            let mut d = [0u8; 8];
...
fn atapi_read(ctba: u64, prdtl: u32, lba: u64, blocks: u32) -> u32 {
    if !backend::media_ready() || blocks == 0 {
        return 0;
    }
    let bs = backend::media_block_size() as u64;
    if bs == 0 {
        return 0;
    }
    ...
    for i in 0..prdtl as usize {
        ...
        while done < dbc {
            if pos == filled {
                ...
                let ok = backend::media_read(cur_lba, bl, stage_ptr());
                ...
                if mb >= 32 {
                    crate::devices::i8042::disarm_auto_key();
                }
                if n % 64 == 0 {
