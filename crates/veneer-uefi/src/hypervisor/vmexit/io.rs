...
use crate::hardware::devices::{acpi_pm, fwcfg, i8042};
use crate::hardware::devices::bus::pci;
use crate::hardware::devices::irq::{pic, pit, rtc};
...
pub unsafe fn handle(vmcb: *mut Vmcb, gprs: &mut GuestGprs) -> Action {
    ...
    if c.exit_info_1 & (1 << 2) != 0 {
        ...
        if is_in {
            ...
            for _ in 0..count {
                if let Some(dst) = crate::guest::boot::guest_mem::to_host(rdi) {
            ...
            for _ in 0..count {
                if forward {
                    if let Some(src) = crate::guest::boot::guest_mem::to_host(rsi) {
    ...
    if is_in {
        if port == VMWARE_BACKDOOR_PORT {
            // Return zeros across the board. The host VMware backdoor
            // would echo "VMXh" in EBX; ours emphatically does not.
            s.rax = 0;
            gprs.rbx = 0;
            gprs.rcx = 0;
            gprs.rdx = 0;
        } else if pit::covers(port) {
            // 8254 PIT — counters 0/1/2 + mode register + system ctl 0x61.
            // Required for Linux TSC calibration.
            s.rax = pit::handle_in(port) as u64;
        } else if pic::covers(port) {
            // 8259A master/slave + ELCR. Even when IO-APIC handles
            // delivery the legacy PIC still has to ack the BIOS-era
            // probe sequence and Linux's mask-everything writes.
            s.rax = pic::handle_in(port) as u64;
        } else if rtc::covers(port) {
            // MC146818 RTC + CMOS RAM (ports 0x70/0x71). Without this
            // Linux prints "Unable to read current time from RTC" and
            // falls back to epoch=0, which breaks time-dependent init.
            s.rax = rtc::handle_in(port) as u64;
        } else if i8042::covers(port) {
            // 8042 PS/2 keyboard controller (0x60/0x64). Ps2KeyboardDxe and
            // the OS keyboard driver probe this; the host-keyboard bridge
            // feeds scancodes into its output buffer.
            s.rax = i8042::handle_in(port) as u64;
        } else if pci::is_config_address_port(port) {
            // Read-back of the latched CONFIG_ADDRESS. Mechanism-#1 probes
            // (Linux pci_check_type1) write a value here then read it back
            // to confirm the port exists before trusting CONFIG_DATA.
            s.rax = pci::read_address(port, width);
        } else if pci::is_config_data_port(port) {
            // PCI configuration mechanism #1 — read currently-latched
            // CONFIG_ADDRESS via the device-specific dispatcher, honouring
            // the byte offset + width so 8/16-bit ID probes read the right
            // slice of the register dword.
            s.rax = pci::read_data(port, width);
        } else if port == fwcfg::DATA_PORT {
            // QEMU fw_cfg data byte (also reached via REP INSB above).
            s.rax = fwcfg::next_byte() as u64;
        } else if acpi_pm::covers(port) {
            // ACPI PM Timer / PM1 block. OVMF stall loops read the 24-bit
            // timer here; without it firmware delays never complete.
            s.rax = acpi_pm::read(port, width);
        } else if port == 0x402 {
            // EDK2 debug-console probe: OVMF reads 0x402 and only enables
            // DEBUG() output if it reads back the Bochs/QEMU debugcon magic
            // 0xE9. Return it so a DEBUG-build OVMF logs to this port.
            s.rax = 0xE9;
        } else if in_com1_range(port) {
            // 16550 register read (full model in `com1`): scratch register,
            // divisor latch, LCR/MCR round-trip, and the internal-loopback
            // path OVMF's PciSioSerialDxe verify exercises.
            s.rax = com1::read(port - COM1_BASE);
        } else if serial_kd::covers(port) {
            // COM2 = Windows KD transport (bidirectional, bridged to WinDbg).
            s.rax = serial_kd::read(port - 0x2F8);
        } else {
            s.rax = SYNTH_IO_DWORD;
        }
    } else {
        // OUT — most ports drop on the floor, but CONFIG_ADDRESS gets
        // latched so the next CONFIG_DATA read knows which device the
        // guest is asking about.
        if pit::covers(port) {
            pit::handle_out(port, s.rax as u8);
        } else if pic::covers(port) {
            pic::handle_out(port, s.rax as u8);
        } else if rtc::covers(port) {
            rtc::handle_out(port, s.rax as u8);
        } else if i8042::covers(port) {
            i8042::handle_out(port, s.rax as u8);
        } else if port == pci::CONFIG_ADDRESS_PORT {
            pci::store_address(s.rax as u32);
        } else if port == fwcfg::SELECTOR_PORT {
            // fw_cfg item select (16-bit), rewinds the data stream.
            fwcfg::select(s.rax as u16);
        } else if pci::is_config_data_port(port) {
            // CONFIG_DATA write — persists the ICH9 PM base OVMF programs.
            pci::write_data(port, s.rax as u32, width);
        } else if acpi_pm::covers(port) {
            acpi_pm::write(port, s.rax, width);
        } else if port == 0x402 {
            // EDK2 ISA debug port (0x402, used by DEBUG-build OVMF): ASCII,
            // accumulate into the line buffer and emit `[guest] <line>`.
            guest_serial_putc(s.rax as u8);
        } else if in_com1_range(port) {
            // 16550 register write. The model owns DLAB/loopback semantics;
            // only a real (non-loopback) THR write produces a byte we mirror
            // to veneer's serial log. Loopback bytes are kept inside the chip
            // (fed back to RBR) so the OVMF verify reads them, not the log.
            if let Some(byte) = com1::write(port - COM1_BASE, s.rax as u8) {
                guest_serial_putc(byte);
            }
        } else if serial_kd::covers(port) {
            // COM2 = Windows KD transport.
            serial_kd::write(port - 0x2F8, s.rax as u8);
        }
    }
