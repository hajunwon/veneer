...
use crate::devices::storage::{ahci, nvme};
use crate::devices::irq::{hpet, ioapic, lapic};
use crate::devices::bus::{nic, vga, xhci};
use crate::devices::tpm;
...
pub unsafe fn handle(vmcb: *mut Vmcb, gprs: &mut GuestGprs) -> Action {
    ...
    if unsafe { crate::hook::dispatch(vmcb, gpa, info1, rip, gprs) } {
    ...
    if (0x7E00_0000..0x8000_0000).contains(&rip) {
        ...
        if n < 8 || n % 8192 == 0 {
            ...
            let clk = crate::clock::now();
            let floor = crate::clock::spin_floor();
...
impl TrapRegion {
    fn for_gpa(gpa: u64) -> Option<Self> {
        ...
        if crate::devices::bus::pci::covers_ecam(gpa) { return Some(Self::Ecam); }
        None
    }
    ...
    fn read(&self, gpa: u64, width: u8) -> u64 {
        match self {
            ...
            Self::Ecam   => crate::devices::bus::pci::ecam_read(gpa, width),
        }
    }
    ...
    fn write(&self, gpa: u64, val: u64, width: u8) {
        match self {
            ...
            Self::Ecam   => crate::devices::bus::pci::ecam_write(gpa, val, width),
        }
    }
}
