...
fn brand_chunk(byte_offset: usize) -> u32 {
    ...
    let src: &[u8] = match active::PROFILE.get() {
        Some(p) => {
            let b = p.hardware.cpu.brand.as_bytes();
...
pub fn effective_core_count() -> u32 {
    let override_cores_minus_1: u8 = active::PROFILE
        .get()
        .map(|p| unsafe {
            core::ptr::addr_of!(p.hardware.cpu.cpuid_cores_override).read_unaligned()
        })
        .unwrap_or(0);
...
fn hide_flags() -> (bool, bool) {
    active::PROFILE
        .get()
        .map(|p| (p.hardware.cpu.hide_hypervisor_bit, p.hardware.cpu.hide_hypervisor_leaf))
        .unwrap_or((true, true))
}
...
fn vendor_buffer() -> [u8; 12] {
    ...
    let src: &[u8] = match active::PROFILE.get() {
        Some(p) => {
            let b = p.hardware.cpu.vendor.as_bytes();
...
pub unsafe fn handle(vmcb: *mut Vmcb, gprs: &mut GuestGprs) -> Action {
    ...
    if !hide_leaf && (HV_LEAF_LOW..=HV_LEAF_HIGH).contains(&leaf) {
        ...
        match leaf {
            ...
            0x4000_0010 => {
                ...
                let host_hz = crate::clock::tsc_freq::host_tsc_freq();
    ...
    if leaf == 1 {
        ...
        if let Some(p) = active::PROFILE.get() {
            ...
            let eax_override = unsafe { core::ptr::addr_of!(cpu.cpuid_eax_v1).read_unaligned() };
            if eax_override != 0 {
                eax = eax_override;
            }
            let brand_idx = unsafe { core::ptr::addr_of!(cpu.cpuid_brand_idx).read_unaligned() };
            if brand_idx != 0 {
