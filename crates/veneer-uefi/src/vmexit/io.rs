...
use super::Action;
use crate::debug::serial_kd;
use crate::devices::{acpi_pm, fwcfg, i8042};
use crate::devices::bus::pci;
use crate::devices::irq::{pic, pit, rtc};
...
pub unsafe fn handle(vmcb: *mut Vmcb, gprs: &mut GuestGprs) -> Action {
    ...
    if c.exit_info_1 & (1 << 2) != 0 {
        ...
        if is_in {
            ...
            for _ in 0..count {
                if let Some(dst) = crate::boot::guest_mem::to_host(rdi) {
            ...
            for _ in 0..count {
                if forward {
                    if let Some(src) = crate::boot::guest_mem::to_host(rsi) {
