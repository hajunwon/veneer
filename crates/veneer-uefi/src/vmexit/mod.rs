...
use crate::svm::vmcb::{exit_code, Vmcb};
...
#[inline]
pub fn advance_rip(state: &mut crate::svm::vmcb::VmcbStateSave, next_rip: u64, fallback_len: u64) {
...
pub unsafe fn dispatch(vmcb: *mut Vmcb, gprs: &mut GuestGprs) -> Action {
    ...
    if code != exit_code::INTR {
        ...
        let floor = if count >= SPIN_THRESHOLD {
            (crate::clock::tsc_freq::host_tsc_freq() / 1_000_000).max(1)
        } else {
            0
        };
        crate::clock::set_spin_floor(floor);
    }
    ...
    {
        let want_native = crate::clock::step_absorbed();
        ...
        if want_native && !is_native {
            ...
            unsafe {
                ...
                c.intercept_vec1 &= !crate::svm::vmcb::intercept_vec1::RDTSC;
                c.intercept_vec2 &= !crate::svm::vmcb::intercept_vec2::RDTSCP;
            }
            crate::sprintln!(
                "[clock] step cluster quiet ({} steps) -> RDTSC NATIVE (no intercept)",
                crate::clock::steps_seen()
            );
    ...
    if matches!(action, Action::Resume) {
        unsafe { crate::devices::irq::inject::stage_pending(vmcb) };
    }
    action
}
...
unsafe fn dispatch_exit(vmcb: *mut Vmcb, gprs: &mut GuestGprs) -> Action {
    ...
    if exception::is_exception(code) {
        ...
        if code == exception::EXCP_BASE + 1 && unsafe { crate::hook::on_single_step(vmcb) } {
