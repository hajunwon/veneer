...
/// Extended Feature Register (MMIO 0x30) value, mirrored into the IVHD "EFR
/// Register Image" so the table and the live register agree. FIRST-GUESS
/// capability set — narrow against the HAL disassembly if needed. Bits:
/// PPRSup(1) XTSup(2) NXSup(3) GTSup(4) IASup(6) GASup(7) HESup(8) PCSup(9)
/// HATS=01(10) GLXSup=001(15).
pub const IOMMU_EFR: u64 = 0x0000_0000_0000_87DE;
...
fn reg_read_u64(aligned_off: u32) -> u64 {
    if aligned_off == REG_EFR {
        return IOMMU_EFR; // RO capability register
    }
