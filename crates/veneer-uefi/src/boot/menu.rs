...
use crate::config::{Config, QuickPolicy};
...
use crate::config::{LogLevel, VmwareBackdoorPolicy};
            ...
            Key::Printable(c) => {
                ...
                match ch {
                    ...
                    's' | 'S' => {
                        ...
                        match crate::identity::nvram_io::save_config(config) {
                            Ok(()) => sprintln!("[adv ] config saved to NVRAM"),
                            Err(e) => sprintln!("[adv ] config save failed: {:?}", e),
                        }
                        crate::identity::active::CONFIG.set(*config);
                        return;
                    }
                    'x' | 'X' => {
                        *config = original;
