...
pub fn poll() {
    ...
    let interval = (veneer_vmm::infra::clock::tsc_freq::host_tsc_freq() / POLL_HZ).max(1);
...
pub fn probe() {
    ...
    match boot::get_handle_for_protocol::<Pointer>() {
        Ok(h) => match boot::open_protocol_exclusive::<Pointer>(h) {
            ...
            Err(e) => sprintln!("[input-probe] SimplePointer present but open failed: {:?}", e),
        },
        Err(_) => sprintln!("[input-probe] SimplePointer: NONE"),
    }
    ...
    match boot::locate_handle_buffer(SearchType::ByProtocol(&TEXT_INPUT_EX_GUID)) {
        Ok(buf) => sprintln!("[input-probe] SimpleTextInputEx PRESENT: {} handle(s)", buf.len()),
        Err(_) => sprintln!("[input-probe] SimpleTextInputEx: NONE"),
    }
}
