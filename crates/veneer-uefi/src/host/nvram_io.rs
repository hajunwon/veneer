...
use veneer_vmm::infra::config::{Config, VENEER_NVRAM_GUID};
...
// A Profile larger than the load buffer would fail the length check every
// boot → never persist → identity churns each boot (fatal for anti-cheat
// caching). Catch any future growth at compile time.
const _: () = assert!(core::mem::size_of::<Profile>() <= PROFILE_BUF_CAP);
...
pub fn load_config() -> Result<Config, NvramError> {
    ...
    let mut c = veneer_vmm::infra::config::DEFAULT;
    unsafe {
        core::ptr::copy_nonoverlapping(
            data.as_ptr(),
